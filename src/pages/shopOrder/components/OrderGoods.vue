<template>
  <div class="v-list">
    <div
      class="v-list-item"
      v-for="(item, index) in orderStore.list"
      :key="item.name"
    >
...
const orderStore = useMyStore();

const props = defineProps({
  list: Array
});
const emits = defineEmits(['increase', 'decrease']);
const onClickIncrease = (index: number) => {
  // emits("increase", index);
  orderStore.increase(index);
};
const onClickDecrease = (index: number) => {
  emits('decrease', index);
  orderStore.decrease(index);
};
</script>
